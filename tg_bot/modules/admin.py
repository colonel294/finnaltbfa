...
def pin(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    is_silent = True
    if len(args) >= 1:
        is_silent = not (args[0].lower() == 'باصدا' or args[0].lower() == 'بلند' or args[0].lower() == 'violent')
...
__help__ = """
 - !ادمینها : این دستور لیست همه ادمین های گپو نشونت میده

*فقط ادمینا:*
 - !پین : بی صدا پیامی که میخوای رو برات پین میکنم.
    از !پین باصدا برای پین معمولی استفاده میکنم 
 - !برداشت : خوب اینم مشخصه ،پیامی که پین شده رو ورمیدارم
 - !لینک: لینک دعوت گپو بهت میدم
 - !ارتقا: کسی که روش ریپلی میکنی رو ادمین میکنم!
 - !برکنار: ادمینی که روش ریپلی میکنی رو برکنار میکنم!
"""

__mod_name__ = "مقامات"

PIN_HANDLER = CommandHandler("پین", pin, pass_args=True, filters=Filters.group)
UNPIN_HANDLER = CommandHandler("برداشت", unpin, filters=Filters.group)

INVITE_HANDLER = CommandHandler("لینک", invite, filters=Filters.group)

PROMOTE_HANDLER = CommandHandler("ارتقا", promote, pass_args=True, filters=Filters.group)
DEMOTE_HANDLER = CommandHandler("برکنار", demote, pass_args=True, filters=Filters.group)

ADMINLIST_HANDLER = DisableAbleCommandHandler("ادمینها", adminlist, filters=Filters.group)

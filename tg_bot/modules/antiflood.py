...
def set_flood(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if len(args) >= 1:
        val = args[0].lower()
        if val == "خاموش" or val == "غیرفعال" or val == "0":
            sql.set_flood(chat.id, 0)
...
یه قابلیت برای حذف آدمایی که گپو به مسلسل میبندن و تند تند پیام میدن!
 ...
 - !مسلسل : تنضیمات وتعداد دقیق پیام های تکرار رو بهتون نشون میده

*فقط ادمینها:*
 - !ضدتکرار <خاموش/عدد>: خاموش و روشن کردن حالت ضد تکرار من!
"""

__mod_name__ = "مسلسل"

FLOOD_BAN_HANDLER = MessageHandler(Filters.all & ~Filters.status_update & Filters.group, check_flood)
SET_FLOOD_HANDLER = CommandHandler("ضدتکرار", set_flood, pass_args=True, filters=Filters.group)
FLOOD_HANDLER = CommandHandler("مسلسل", flood, filters=Filters.group)

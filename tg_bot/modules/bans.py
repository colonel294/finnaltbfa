...
__help__ = """
 - !خروج : هرکی این دستورو بنویسه اخراج میشه

*فقط ادمینها:*
 - !بن یا /ban  <آیدی>: 
 یه کاربر رو بن میکنه حالا یا با ریپلی یا آیدی
 - /tban <آیدی> x(دقیقه/ساعت/روز): 
 کاربر رو به مدت x از گپ اخراج میکنه 
 - !آزادکن یا /unban <آیدی> :
 کاربر رو از بلک لیست گپ درمیاره .یا ریپلی یا آیدی
 - !اخراج <آیدی> : کاربر رو اخراج میکنه ! باز میتونه برگرده !!
"""
...
KICK_HANDLER = CommandHandler("اخراج", kick, pass_args=True, filters=Filters.group)
UNBAN_HANDLER = CommandHandler(["آزادکن", "unban"], unban, pass_args=True, filters=Filters.group)
KICKME_HANDLER = DisableAbleCommandHandler("خروج", kickme, filters=Filters.group)

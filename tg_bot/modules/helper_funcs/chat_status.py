...
def bot_can_delete(func):
    ...
    def delete_rights(bot: Bot, update: Update, *args, **kwargs):
        if can_delete(update.effective_chat, bot.id):
            ...
            update.effective_message.reply_text("نموتانم پیام پاک کنم! "
                                                "لطفا چک کن ببین اجازه هام بازه؟")
...
def can_pin(func):
    ...
    def pin_rights(bot: Bot, update: Update, *args, **kwargs):
        if update.effective_chat.get_member(bot.id).can_pin_messages:
            ...
            update.effective_message.reply_text("نمیتونم پیامتو پین کنم"
                                                "لطفا چک کن ببین اجازه هام بازه؟")
...
def can_promote(func):
    ...
    def promote_rights(bot: Bot, update: Update, *args, **kwargs):
        if update.effective_chat.get_member(bot.id).can_promote_members:
            ...
            update.effective_message.reply_text("نمیتونم کسیو مقام بدم یا برکنار کنم "
                                                "لطفا چک کن ببین اجازه هام بازه؟")
...
def can_restrict(func):
    ...
    def promote_rights(bot: Bot, update: Update, *args, **kwargs):
        if update.effective_chat.get_member(bot.id).can_restrict_members:
            ...
            update.effective_message.reply_text("من نمیتونم کسیو محدود کنم"
                                                "لطفا چک کن ببین اجازه هام بازه؟")
...
def bot_admin(func):
    ...
    def is_admin(bot: Bot, update: Update, *args, **kwargs):
        if is_bot_admin(update.effective_chat, bot.id):
            ...
            update.effective_message.reply_text("من ادمین نیستم چرا به من میگی")
...
def user_admin(func):
    ...
    def is_admin(bot: Bot, update: Update, *args, **kwargs):
        ...
        if user and is_user_admin(update.effective_chat, user.id):
            ...
            update.effective_message.reply_text("شما کی باشی که به من دستور بدی؟؟")

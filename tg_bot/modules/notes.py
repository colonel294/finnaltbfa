...
__help__ = """
هشتگ کن فایلاتو !!! یه قابلیت بسیار کارامد میتونه باشه
دیدی یه سری افراد برنامه یا فایل خاصی که مرتبط با گپتونه رو میخوان؟؟  خب با این قابلیت فایل هاتو هشتگ بزن که اگه کسی لازم داشت هشتگشو بنویسه 
و من براش برنامه رو میفرستم

 - !دانلود <نام هشتگ>: محتوای که برای این نام هشتگ نوشته شده رو نشونت میدم
 - #<نام هشتگ>: 
 دستورش دقیقا مثل بالایی عمل میکنه
 - !هشتگها : لیست همه هشتگ های سیو شده داخل گپ



*فقط ادمینها:*
 - !ذخیره <نام هشتگ> <محتوا>:
  ،توجه داشته باش هشگ بدون فاصلس 
 میدونم که نباید اینو بگم ولی همه که مثل تو باهوش نیستن😄
 - !ذخیره <نام هشتگ> : پیامی که روش ریپلی بزنی رو به عنوان محتوا ذخیره میکنم!
 - !حذف <نام هشتگ>: هشتگ مورد نظر رو از بین میبرم!
"""

__mod_name__ = "هشتگ"

GET_HANDLER = CommandHandler("دانلود", cmd_get, pass_args=True)
HASH_GET_HANDLER = RegexHandler(r"^#[^\s]+", hash_get)

SAVE_HANDLER = CommandHandler("ذخیره", save)
DELETE_HANDLER = CommandHandler("حذف", clear, pass_args=True)

LIST_HANDLER = DisableAbleCommandHandler(["هشتگها", "suver"], list_notes, admin_ok=True)
